...
const OX = new OnirixSDK(
  'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOjQyMzMyLCJwcm9qZWN0SWQiOjkwMzMyLCJyb2xlIjozLCJpYXQiOjE3MjgyNjYyMTl9.0P6W6AE3NWb7ymImUnWNrrdlx5tOjr5mpB3kXE0wE7c'
);
...
function onTouch(touchPos) {
  const docentCharacter = localStorage.getItem('docentCharacter') || 'bear';
  ...
  const selectedModel = modelMap[docentCharacter] || 'bearModel'; 
  ...
  if (intersects.length > 0) {
    ...
    model.setAttribute('scale', `${scaleValue * 0.5} ${scaleValue * 0.5} ${scaleValue * 0.5}`);
